...
# Modifica la funzione in Sezione 3
def play_sound(file_name):
    try:
        # Aggiungiamo "sounds/" prima del nome del file
        # Nota: assicurati che il nome file passato non abbia spazi (es: "win.mp3")
        with open(f"sounds/{file_name}", "rb") as audio_file:
            audio_bytes = audio_file.read()
            st.audio(audio_bytes, format="audio/mp3", autoplay=True)
    except FileNotFoundError:
        st.warning(f"Audio file 'sounds/{file_name}' not found.")
    except Exception as e:
        pass # Ignora errori audio per non bloccare l'app
